...
from believe import Believe, DefaultAioHttpClient, AsyncBelieve
...
def client(request: FixtureRequest) -> Iterator[Believe]:
    strict = getattr(request, 'param', True)
    if not isinstance(strict, bool):
      raise TypeError(f'Unexpected fixture parameter type {type(strict)}, expected {bool}')

    with Believe(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client :
        yield client

@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncBelieve]:
    ...
    async with AsyncBelieve(base_url=base_url, api_key=api_key, _strict_response_validation=strict, http_client=http_client) as client :
        yield client